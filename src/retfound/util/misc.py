...
def cleanup_distributed():
    """Synchronize and release the distributed process group."""
...
def save_checkpoint_on_master(checkpoint, path):
    """Atomically save a checkpoint so other ranks never see a partial file."""
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, mode):
    ...
    if loss_scaler is not None:
        ...
        for checkpoint_path in checkpoint_paths:
            ...
            save_checkpoint_on_master(to_save, checkpoint_path)
    else:
        if mode == 'best':
            to_save = {
                'model': model_without_ddp.state_dict(),
            ...
            save_checkpoint_on_master(
                to_save,
                os.path.join(args.output_dir, args.task, "checkpoint-latest.pth"),
            )
